...
export function RecordTab({
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => setDebouncedModalSearchText(modalSearchText),
      300,
    );
    return () => clearTimeout(timer);
  }, [modalSearchText]);
  ...
  const filteredPokeList = useMemo(
    () =>
      fullPokeList.filter(
        (poke) =>
          poke.name.includes(debouncedSearchText) ||
          poke.hira.includes(debouncedSearchText),
      ),
    [debouncedSearchText],
  );
  ...
  const filteredModalPokeList = useMemo(
    () =>
      fullPokeList.filter(
        (poke) =>
          poke.name.includes(debouncedModalSearchText) ||
  ...
  const handleSaveModal = () => {
    if (tempMyPartyIds.length !== 6)
      return showToast("自分のパーティを6匹選んでください");
  ...
  const renderPickButton = (
    ...
    return (
      ...
      >
        {/* ★追加: 相手のポケモンの場合のみ、左上に削除(×)ボタンを表示 */}
        {/* ★修正: 相手のポケモンの場合のみ、左上に削除(－)ボタンを表示 */}
        {isOpp && (
          <button
            className="delete-badge"
            onClick={(e) => {
              e.stopPropagation(); // 選出ボタン自体のクリックと重ならないようにする
              handleToggleOpp6(poke.id); // パーティから解除
            }}
            aria-label="削除"
          >
            {/* × の代わりに綺麗なマイナスのアイコンを使用 */}
        ...
        >
          <div
            className={`pick-badge ${badgeClass}`}
            style={{
    ...
    <div role="tabpanel">
      ...
      >
        <div className="section-header" style={{ marginBottom: "12px" }}>
          ...
          <button
            ...
            onClick={() => {
              ...
              setIsSeasonEditing(!isSeasonEditing);
            }}
            style={{
              backgroundColor: isSeasonEditing ? "#1A73E8" : "var(--divider)",
              color: isSeasonEditing ? "white" : "var(--text-sub)",
            }}
          >
        ...
        <div style={{ position: "relative" }}>
          <input
            ...
            onFocus={() => {
              if (isSeasonEditing) setIsSeasonDropdownOpen(true);
            }}
            onBlur={() => setTimeout(() => setIsSeasonDropdownOpen(false), 200)}
            style={{
              margin: 0,
              backgroundColor: isSeasonEditing ? "#FFFFFF" : "var(--app-bg)",
              border: isSeasonEditing
                ? "2px solid #1A73E8"
                ...
                {availableSeasons.map((s, idx) => (
                  <div
                    ...
                    onMouseDown={(e) => {
                      e.preventDefault();
                      setRecordSeason(s);
                      localStorage.setItem("vgc_season", s);
        ...
        {!isSeasonEditing && recordSeason === "" && (
          ...
          >
            ※「変更する」を押してシーズンを入力してください
          </div>
        )}
      </div>

      {/* 相手のパーティ選択（6匹揃ったら丸ごと非表示にする） */}
      ...
      <div className="md-card">
        ...
        <div className="poke-container" style={{ marginTop: "12px" }}>
          ...
          {Array.from({ length: Math.max(0, 6 - selectedIds.length) }).map(
            (_, i) => (
              <div
                key={`filler-opp-${i}`}
        ...
        <div className="poke-container">
          {myPartyList.length === 0 ? (
            ...
            <>
              ...
              {Array.from({ length: Math.max(0, 6 - myPartyList.length) }).map(
                (_, i) => (
                  <div
                    key={`filler-my-${i}`}
                    style={{ visibility: "hidden", pointerEvents: "none" }}
      ...
      >
        <button
          className={`result-btn win ${matchResult === "勝ち" ? "selected" : ""}`}
          onClick={() => setMatchResult("勝ち")}
        >
          勝ち
        </button>
        <button
          className={`result-btn lose ${matchResult === "負け" ? "selected" : ""}`}
          onClick={() => setMatchResult("負け")}
        >
          負け
        </button>
      </div>
          ...
          <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                ...
                >
                  {filteredModalPokeList.map((poke) =>
                    renderButton(
                      poke,
                      tempMyPartyIds,
                      handleToggleModalPick,
